...
export const getDiff = async () => {
  const proc = cli(['git', 'diff', '--cached', '--patch', '--exclude=**/bun.lockb', '--exclude=**/package-lock.json', '--exclude=**/pnpm-lock.yaml'])
  return new Response(proc.stdout).text().then(t => t.trim());
}
...
export const gitUntracked = async () => {
  ...
  return new Response(proc.stdout).text().then(t => t.trim().split('\n').filter(Boolean));
}
...
export const getCommitMessage = async () => {
  ...
  const res = await ai.fetch<{ message: string }>(gitCommitPrompt, JSON.stringify({
    VCS_BRANCH: branch,
    VCS_DIFF: diff
  }))
